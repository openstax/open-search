...
RSpec.describe EnqueueIndexJobs do
  ...
  let(:release_ids)       { %w(1 2) }
  let(:releases)          { release_ids.map { |release_id| Rex::Release.new id: release_id, data: {}, config: nil } }
  ...
  context "new book listings" do
    ...
    before do
      ...
      allow_any_instance_of(Rex::Releases).to receive(:load_releases).and_return(releases)
  ...
  context "enqueued book listings not existing in rex releases" do
    ...
    let(:book1_bk_index)    { double(index_id: '1', in_demand: true, indexing_strategy_name: 'I2') }
    let(:book1_pg_index)    { double(index_id: '1', in_demand: true, indexing_strategy_name: 'I3') }
